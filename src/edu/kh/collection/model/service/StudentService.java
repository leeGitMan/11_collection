package edu.kh.collection.model.service;

import java.util.ArrayList;
import java.util.InputMismatchException;
import java.util.List;
import java.util.Scanner;

import edu.kh.collection.model.vo.Student;





public class StudentService {
	
	private Scanner sc = new Scanner(System.in);
	
	
	// 학생 정보를 저장할 List를 생성 (객체 배열의 Upgrade버전)
	
	// java.util.List 인터페이스 : List에 반드시 필요한 필수 기능을 모은 인터페이스
	// * 인터페이스는 객체 생성이 안됨, 변수로써의 역할은 가능했다.
	
	// java.util.ArrayList : 배열 형태의 List (가장 대표적인 List 자식 클래스)
	
	
	// ArrayList() 기본생성자 : --> 기본 크기 10짜리 리스트 생성
	// 							 List크기가 늘었다 줄었다 하기에 큰 의미가 없다
	
	// ArrayList(용량) : 용량 만큼의 리스트 생성
	//					너무 큰 값을 적으면 메모리 소모가 큼
	
	// List studentList = new ArrayList();
	   private List<Student> studentList = new ArrayList<Student>(); // 검색 조회 효율적
	// Student로 저장되는 타입이 제한된 리스트 생성
	// --> Student만 저장가능하고, 이 안에 있는 모든 게 Student다
	// List<Student> studentList = new Linked<Student>(); // 추가, 삭제, 수정 효율적
	
	
	public StudentService() {
		studentList.add(new Student("홍길동", 25, "서울시 중구", 'M', 90)); // 반환 타입(리턴 타입) void
		studentList.add(new Student("고영희", 23, "경기도 안산시", 'F', 100));
		studentList.add(new Student("김아지", 30, "서울시 강남구", 'M', 80));
		studentList.add(new Student("오미나", 27, "충북 청주시", 'F', 90));
		studentList.add(new Student("박주희", 24, "서울 서대문구", 'F', 70));
	}

	
	public void ex() {
		// List 테스트
		
		// List.add(Object e) : 리스트에 객체를 추가하는 용도
		// * 매개변수 타입이 Object 인데 == 모든 객체를 매개변수로 전달할 수 있다는 뜻
		// (매개변수 Object == 최상위 부모 참조변수 == 다형성 적용이 가능하다)
		
		studentList.add(new Student());// 0번 인덱스
		// studentList.add(sc);// 1번 인덱스
		// studentList.add("흠");// 2번 인덱스
		// studentList.add(new Object());// 3번 인덱스
		
		//--> 다 다른 여러타입의 데이터 저장 가능
		
		
		// (반환형)
		
		// List.get(index i) : List에서 i번 째 인덱스에 있는 객체(Object)를 반환함 // 리턴타입 E --> 주어진 객체를 반환
		// 반환형이 Object == 모든 객체를 반환할 수 있다는 뜻
		
		// System.out.println(studentList.get(2).toString());
		
		// [Student의 이름만 얻어오기]
		// Student 객체가 맞는지 instanceof로 확인하고
		// 다운 캐스팅 이유 : get(int index) 메서드의 반환형이 Object이기 때문에
		// 다운 캐스팅을 하고 Student 기능을 사용할 수 있다.
		// if(studentList.get(0) instanceof Student) {
		//	System.out.println(((Student)studentList.get(0)).getName());	
	//	}
		
		
		
	
		
		// -> 위에 코드는 길고 복잡함
		// -> 컬렉션의 장점인, 여러 객체 저장이 방해될 정도
		// -> 그래서 나타난 게
		
		// **** 제네릭스(Generics) ****
		// <> 이렇게 쓴다
		// -> 컬렉션에 저장되는 객체 타입을 한 가지로만 제한
		
	//	System.out.println(studentList.get(0).getName());
		
		
		
	}
	
	// 알트 쉬프트 제이
	
	/** @author 이현경
	 * 메뉴 출력 메서드
	 * 
	 * 
	 */
	public void displayMenu() {
		
		int menuNum = 0;
		
		do {
			System.out.println("\n================== 학생관리 프로그램 ==================");
			System.out.println("1. 학생 정보 추가");
			System.out.println("2. 학생 전체 조회");
			System.out.println("3. 학생 정보 수정");
			System.out.println("4. 학생 정보 제거");
			// 1 ~ 4번까지 CRUD 
			System.out.println("5. 이름으로 검색(완전일치)");
			System.out.println("6. 이름으로 검색(포함일치)");
			System.out.println("0. 프로그램 종료");
			
			
			System.out.print("\n메뉴 번호 선택 >>");
			
			try {
				
				menuNum = sc.nextInt();
				System.out.println(); // 줄 바꿈
				
				switch(menuNum) {
				case 1: System.out.println(addStudent()); break;
				case 2: selectAll(); break;
				case 3: System.out.println(updateStudent()); break;
				case 4: System.out.println(removeStudent()); break;
				case 5: searchName1(); break;
				case 6: searchName2(); break;
				case 0: System.out.println("프로그램 종료."); break;
				default : System.out.println("메뉴 번호만 입력해주세요."); break;
				
				}
				
			}
			catch(InputMismatchException e) {
				
				System.out.println("\nerror : 입력 형식이 유효하지 않습니다. 다시 입력해");
				sc.nextLine(); // --> 입력 버퍼에 남아있는 잘못된 문자열 제거용
				menuNum = -1; // 첫 반복 시, 잘못 입력한 경우 menuNum이 0을 가지고 있어서 종료됨
							// 이를 방지하고자 임의의 값을 넣음.
			}
		}while(menuNum != 0);
	
	}
	
	
	
	/**
	 * @author 이현경
	 * @return
	 * 1. 학생 정보 추가 메서드
	 *  - 추가 성공 시, "성공"으로 실패 시, "실패"로 문자열 반환
	 * @throws InputMismatchException
	 */
	public String addStudent() throws InputMismatchException {
		
		System.out.println("=== 학생 정보 추가 ===");
		
		System.out.print("1. 이름 입력 : ");
		String name = sc.next();
		
		System.out.print("2. 나이 입력 : ");
		int age = sc.nextInt();// ㅁㄴㅇ
		
		System.out.print("3. 지역 입력 : ");
		String region = sc.next();
		
		System.out.print("4. 성별 입력 : ");
		char gender = sc.next().charAt(0);
						// String -> char
		
		System.out.print("5. 점수 입력 : ");
		int score = sc.nextInt();
		
		
		// 입력 다 받았으니, Student 객체 생성 후, 리스트에 추가할 일이 남음
		// add는 boolean 형태이다
		// 값을 true로 반환
		
		
		if(studentList.add(new Student(name,age,region,gender,score))) {
			return "성공";
		}
		else {
			return "실패";
		}
	}
	
	
	
	
	/**
	 * @author 이현경
	 * 2. 학생 전체 조회 메서드
	 * 향상된 for문 사용
	 * 
	 */
	public void selectAll() { // 전체 조회
		
		
		// List에는 인덱스가 있다. ( 0번 부터 시작 )
		// int List.size로 리스트에 저장된 데이터 개수를 얻어옴 
		// -> 배열명.length 대신 사용
		
		System.out.println("==== 학생 전체 조회 ====");
		// studentList가 비어있는 경우 "학생 정보가 없습니다." 출력
		
		// List가 비어있는지 확인 하는 방법
		// boolean자료형에 List.isEmpty() : 비어있으면 true를 반환
		
		//if(studentList.size() == 0) {
		//	System.out.println("학생 정보가 없습니다.");
		//}
		
		if(studentList.isEmpty()) { // studentList가 비어있으면 true 반환으로 밑에 구문 실행
			System.out.println("학생 정보가 없습니다.");
			
			return; // 반환형 없는(void) return 구문은 현재 메서드를 종료하고 호출한 곳으로 돌아감
					// 자동으로 else문을 쓴 것 같은 효과
			
		}
		// 일반 for문
		//for(int i = 0; i<studentList.size(); i++) {
		//	System.out.println(studentList.get(i));
		//}
		
		// 향상된 for문 (for each문)
		// - 컬렉션, 배열의 모든 요소를 순차적으로 반복 접근 가능 
		// (순차적 : 0번 부터 1씩 증가하며 마지막 요소 까지 접근)
		
		// [작성법]
		// for(컬렉션 or 배열에서 꺼낸 한개의 요소를 저장할 변수 : 컬렉션명 or 배열명){}
		
		// std에 studentList의 각각 인덱스들이 순차적으로 저장됨
		int index = 0;
		for(Student std : studentList) {
			// std에는 for문 반복 시 마다 0,1,2... 인덱스 요소들이 한 번씩 저장됨
			System.out.print((index++) + "번 : "); 
			System.out.println(std);
		}
		
	}
	
	
	
	
	
	
	
	/**@author 이현경
	 * 3.학생 정보 수정 메서드
	 * @return
	 */
	public String updateStudent() throws InputMismatchException {
		
		// List.set(index, Student e)
		// -> List의 index 요소를 전달 받은 e로 변경
		// -> 반환값 존재 Student == 변경 전 Student 객체가 담겨있음 --> 얘가 반환됨
		System.out.println("===== 학생 정보 수정 =====");
		
		System.out.print("인덱스 번호 입력 : ");
		int index = sc.nextInt();
		sc.nextLine();
		
		// 학생 정보가 studentList에 있는가?
		if(studentList.isEmpty()) {
			return "입력된 학생 정보가 없습니다.";
			// 입력된 숫자(인덱스)가 0보다 혹시 작은가?(음수)
		}else if(index < 0) {
			return "음수는 입력할 수 없습니다.";
			// 입력된 숫자(인덱스)가 studentList 범위 내 인덱스 번호인가?
			// + 문자열을 입력한 경우 --> throws로 예외처리 --> 메서드 명 옆에 했음.	
		}else if(index >= studentList.size()){
			return "범위를 넘어선 값을 입력할 수 없습니다.";
		}else { // 정상인 경우
			// 수정 코드 작성
			System.out.println(index + "번째 인덱스에 저장된 학생 정보");
			System.out.println(studentList.get(index));
			
			System.out.print("이름 :");
			String name = sc.next();
			
			System.out.print("2. 나이 입력 : ");
			int age = sc.nextInt();// ㅁㄴㅇ
			sc.nextLine();
			
			System.out.print("3. 지역 입력 : ");
			String region = sc.nextLine();
			
			System.out.print("4. 성별 입력(M/F) : ");
			char gender = sc.next().charAt(0);
							// String -> char
			
			System.out.print("5. 점수 입력 : ");
			int score = sc.nextInt();
			
			// 입력 받은 index번 째 새로운 학생 정보를 세팅 == 수정
			// 이때, index번째에 있던 기존 학생 정보가 반환됨
			
			Student temp = studentList.set(index, new Student(name, age, region, gender, score)); 
			// 기존의 객체를 반환 하기에 temp에 담아둔다.
			
			return temp.getName() + "의 정보가 변경되었습니다.";
			
			
		}
		
		
	}
	
	
	
	
	
	/**@author 이현경
	 * 4. 학생 정보 제거 메서드
	 * 
	 * @return
	 */
	public String removeStudent() throws InputMismatchException{
		
		// List.remove(int index)
		// 리스트에서 index번 째 요소를 제거함
		// 반환형이 존재
		// 제거된 요소가 반환됨
		// * List는 중간에 비어있는 인덱스가 없게 하기 위해서
		// remove() 동작 시 뒤쪽 요소를 한 칸씩 당겨온다.
		
		System.out.println(" ===== 학생 정보 제거 ===== ");
		
		System.out.println(" 인덱스 번호 입력 ");
		int index = sc.nextInt();
		sc.nextLine();

		// 학생 정보가 studentList에 있는가?
		if(studentList.isEmpty()) {
			return "입력된 학생 정보가 없습니다.";
			// 입력된 숫자(인덱스)가 0보다 혹시 작은가?(음수)
		}else if(index < 0) {
			return "음수는 입력할 수 없습니다.";
			// 입력된 숫자(인덱스)가 studentList 범위 내 인덱스 번호인가?
			// + 문자열을 입력한 경우 --> throws로 예외처리 --> 메서드 명 옆에 했음.	
		}else if(index >= studentList.size()){
			return "범위를 넘어선 값을 입력할 수 없습니다.";
		}else {
			// 학생 정보 제거
			
			System.out.print("정말 삭제할거야? (Y/N) : ");
			char ch = sc.next().toUpperCase().charAt(0);
						//String -> 대문자 String -> 대문자 0번째 인덱스 따오기
						//String.toUpperCase() : 문자열을 대문자로 변경
						//String.toLowerCase() : 문자열을 소문자로 변경
			
			if(ch == 'Y') { // char형이라서 == 가능함
				Student temp = studentList.remove(index);
				return temp.getName() + "의 정보가 제거되었습니다.";
			}else {
				return "취소";
			}
		}
		
		
		
		
		
	}

	
	
	/**@author 이현경
	 * 5. 이름이 일치하는 학생을 찾아서 조회하는 메서드
	 * 
	 */
	public void searchName1() {
		
		System.out.println("===== 학생 검색(이름 완전 일치) =====");
		System.out.print("검색할 이름 입력 : ");
		String input = sc.next();
		
		boolean flag = true; // flag가 true인 경우에 ==> 검색결과가 없다고 내보낼거임
		
		for(Student std : studentList) {
			
			if(input.equals(std.getName())) { // 이름이 일치하는 경우
				System.out.println(std);
				flag = false;
			}
		}
		if(flag) {
			System.out.println("검색 결과가 없습니다.");
		}
	}
	
	
	/**@author 이현경
	 * 6. 이름의 특정 문자열이 포함되는 학생을 찾아서 조회하는 메서드
	 * 
	 */
	public void searchName2() {
		
		System.out.println("===== 학생 검색(문자열 포함) =====");
		System.out.print("이름에 포함될 문자열 입력 : ");
		String input = sc.next();
		
		boolean flag = true; // flag가 true인 경우에 ==> 검색결과가 없다고 내보낼거임
		
		for(Student std : studentList) {
			
			// contains : 포함
			// boolean 자료형태이다
			// 포함되어있으면 true 아니면 false
			// String.contains(문자열) : String에 문자열이 포함되어있으면 true
			if(std.getName().contains(input)) { // 포함되어이으면 true 아니면 false
				System.out.println(std);
				flag = false;
			}
		}
		if(flag) {
			System.out.println("검색 결과가 없습니다.");
		}
		
	}
	
	
	
	
}
